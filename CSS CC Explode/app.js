...
(function (root, factory) {
    ...
    if (disObj.type === "self-contained") {
      ...
      if (
        ...
        document.querySelector(
          "[data-dis-id = '" + el.dataset.disContainerId + "']"
        )
      ) {
    ...
    if (visibleWidth !== lastWidth) {
      if (
        ...
        if (dimensions.overflowingTop && dimensions.overflowingRight) {
          // Top right
          screenshotData = disObj.scrnCtx.getImageData(
            visibleWidth,
            choppedTop,
            widthDiff,
            visibleHeight
    ...
    if (visibleHeight !== lastHeight) {
      if (
        ...
        if (dimensions.overflowingTop && dimensions.overflowingLeft) {
          ...
          screenshotData = disObj.scrnCtx.getImageData(
            choppedLeft,
            disObj.actualHeight - lastHeight,
            visibleWidth,
            heightDiff
          ).data;
  ...
  function getScreenshot(disObj) {
    html2canvas(disObj.elem, { scale: 1 }).then((canvas) => {
      ...
      if (typeof disObj.canvas === "undefined") {
        ...
        disObj.canvas.className = "disParticleCanvas";
  ...
  function createParticle(
    ...
    if (
      ...
      ((localX < borderRadius &&
        ...
        borderRadius <
          Math.sqrt(
            Math.pow(borderRadius - localX, 2) +
              Math.pow(borderRadius - localY, 2)
          )) || // Top left
        (localX > disObj.actualWidth - borderRadius &&
          localY < borderRadius &&
          borderRadius <
            Math.sqrt(
              Math.pow(localX - (disObj.actualWidth - borderRadius), 2) +
                Math.pow(borderRadius - localY, 2)
            )) || // Top right
        (localX > disObj.actualWidth - borderRadius &&
          localY > disObj.actualHeight - borderRadius &&
          borderRadius <
            Math.sqrt(
              Math.pow(localX - (disObj.actualWidth - borderRadius), 2) +
                Math.pow(localY - (disObj.actualHeight - borderRadius), 2)
            )) || // Bottom right
        (localX < borderRadius &&
          localY > disObj.actualHeight - borderRadius &&
          borderRadius <
  ...
  function animateParticles(disObj) {
    ...
    for (
      let i = 0;
      disObj.particleArr.length > 0 && i < disObj.particleArr.length;
      i++
    ) {
      let percent =
        (Date.now() - disObj.particleArr[i].startTime) /
        disObj.animationDuration;
